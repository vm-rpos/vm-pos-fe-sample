...
const StockoutOrderPage = () => {
  ...
  const fetchOrders = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}?orderType=stockoutOrder`);
  ...
  const handleEdit = (orderId) => {
    ...
    if (orderToEdit) {
      setDestination(orderToEdit.destination || '');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!destination) {
        throw new Error("Destination is required for stockout orders!");
      }
...
export default StockoutOrderPage;